...
export default function FLightSearcher({ airports, facade, chooseFlight }) {
  ...
  const [FlightSets, setSets] = useState([]);
  ...
  function addArrCity(flight) {
    let city = "CCC";
    airports.forEach((airport) => {
      if (airport.code === flight.arrival.iata) {
        console.log("CIty found: " + airport.city);
                  ...
                  {resultList.map((s) => (
                    <div>
                      ...
                      <button
                        ...
                        onClick={(e) => clickOnFlight(e, s)}
                        key={s.id}
                      >
                        {s.flights.map((f) => (
                          <Flight
                            key={s.id + f.flight.iata}
                            flight={f}
                            airports={airports2}
                            onFocus={(e) => {
